# Contribuindo com o SGCA

Obrigado pelo interesse em contribuir com o **SGCA — Sistema de Gestão de Contratos e Atas**!

## Reportando bugs

Abra uma [issue](https://github.com/devtulio/sgca/issues/new) com:
...
3. Faça suas alterações em `SGCA.html` (frontend) e/ou `server.py` (backend)
...
5. Atualize a documentação quando a mudança for relevante para o usuário final:
   ...
   - `SGCA_VERSION` em `SGCA.html` e comentário de versão no topo de `server.py`
...
- **Frontend single-file** — `SGCA.html` contém HTML, CSS e JS num único arquivo, sem build step
