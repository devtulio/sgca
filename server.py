# SGCA v0.40.12 — Servidor local: SQLite, autenticação, REST API, proxy CNPJ/BCB, e-mail SMTP, backup automático
import http.server
import os
import json
import sqlite3
...
def _smtp_cfg_build(cfg, default_name='SGCA'):
...
class SGCAHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def _route_get(self, p, qs, s):
        ...
        # Configurações do sistema
            ...
            result = _settings_para(result, s)
        ...
        # Backup
            ...
            try:
                path = sgx_base.pick_folder_dialog("Selecione a pasta de backup do SGCA")
    ...
    # Espelha o SGCD; ver sgx_base.classificar_merge. SGCA não guarda vínculos
    # locais dentro do JSON do fornecedor (contratos referenciam por id), então o
    # upsert é o merge simples. Compliance (ceis/cnep/sancoes) vem do SGCD e é só
    # carregado aqui (a tela não edita).
    def _export_fornecedores(self):
        ...
        payload = {'_sgx': 'SGCA', 'tipo': 'fornecedores', 'exportedAt': _now(), 'fornecedores': itens}
    ...
    def _forn_upsert(self, conn, local, item):
        ...
        if local:
            ...
            merged = {**base, **canon}   # preserva campos locais não-canônicos
        else:
            fid = str(uuid.uuid4())
            merged = dict(canon)
        merged['id'] = fid
        ...
        conn.execute(
            ...
            (fid, json.dumps(merged, ensure_ascii=False),
             merged.get('cnpj'), merged.get('razao') or merged.get('razao_social'),
             merged.get('updatedAt'), fid))
    ...
    def _send_email(self, data, sess):
        ...
        if not (cfg.get('smtp_host') and cfg.get('smtp_user')):
            raise ValueError('SMTP não configurado. Configure em Configurações → Comunicação (ou no seu perfil).')
...
if __name__ == '__main__':
    _selecionar_modo()
    sgx_base.instalar_captura_de_falhas(_DATA_DIR, 'SGCA')  # crash log que sobrevive à janela fechar
    _check_db_integrity()
    ...
    try:
        ...
        sgx_base.servir_wsgi(SGCAHandler, '', PORT)
    except KeyboardInterrupt:
        print('\n  Encerrando servidor...')
    except Exception:
        import traceback as _tb
        _log.error('Servidor caiu (serve_wsgi): %s', _tb.format_exc())
        print('\n  ERRO FATAL no servidor — registrado em SGCA_crash.log.')
