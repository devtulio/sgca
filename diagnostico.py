# SGCA — Diagnóstico e Correção Automática de Rede
...
def info_maquina():
    ...
    if ip_local not in ('não detectado', '127.0.0.1'):
        print(f'       \033[96mEndereço para outros computadores: http://{ip_local}:{PORT}/SGCA.html\033[0m')
...
def checar_porta():
    ...
    if em_uso:
        ...
        linha(f'Porta {PORT} livre — servidor não está rodando', None,
              'Inicie o servidor pelo Iniciar SGCA.bat para testar o acesso pela rede.')
...
def checar_perfil_rede():
    ...
    if publica:
        linha('Perfil de rede', False, f'Detectado como "Pública" — o Windows bloqueia conexões de entrada por padrão nesse perfil.',
              'Isso impede outras máquinas de acessar o SGCA mesmo com a regra de firewall liberada.')
        if is_admin():
...
def checar_firewall():
    ...
    # Verifica TODAS as regras chamadas "SGCA Servidor" (podem existir duplicadas/quebradas de instalações antigas)
    detalhe = ps(
    ...
    try:
        ...
        if r == 0:
            linha('Acesso pelo IP da rede (a partir desta máquina)', True,
                  f'http://{ip_local}:{PORT}/SGCA.html acessível')
        else:
...
def resumo(ip, servidor_ativo, regra_fw, perfil_ok):
    ...
    if not servidor_ativo:
        problemas.insert(0, 'Servidor não está rodando — inicie pelo Iniciar SGCA.bat')
    ...
    if not problemas:
        ...
        print(f'  \033[96m    http://{ip}:{PORT}/SGCA.html\033[0m')
    else:
...
if __name__ == '__main__':
    ...
    print()
    print('  ╔══════════════════════════════════════════════════╗')
    print('  ║   SGCA — Diagnóstico e Correção de Rede          ║')
    print('  ╚══════════════════════════════════════════════════╝')
