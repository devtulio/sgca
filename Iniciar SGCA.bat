...
echo  Extraindo Python embarcado incluido no SGCA ^(primeira execucao apenas^)...
echo.
...
for /f "tokens=5" %%P in ('netstat -ano ^| findstr LISTENING ^| findstr ":3002 "') do taskkill /F /PID %%P >nul 2>&1
start "SGCA - Servidor local" "%PYEXE%" "%~dp0server.py"
